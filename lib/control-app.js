...
module.exports = function (config, cb) {
  ...
  app.expose(config.project, 'title')

  var frameworksDir = path.resolve(__dirname, '../frameworks')
  app.set('views', frameworksDir)
  app.set('view engine', 'html')
  app.engine('html', require('hbs').__express)

  app.use(function (req, res, next) {
    res.locals.title = config.project
  ...
  // airtap files
  app.use('/__zuul', express.static(frameworksDir))
  ...
  router.get('/__zuul/client.js', function (req, res, next) {
  ...
  router.get('/__zuul/test-bundle.map.json', function (req, res, next) {
    if (!map) {
  ...
  router.get('/__zuul/test-bundle.js', function (req, res, next) {
    res.contentType('application/javascript')
