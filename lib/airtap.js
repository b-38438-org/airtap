...
inherits(Zuul, EventEmitter)

Zuul.prototype.browser = function (opts) {
  opts = Object.assign({}, this._config, opts)
  this._browsers.push(SauceBrowser(opts))
}
...
Zuul.prototype.run = function (cb) {
  ...
  createControlServer(config, function (err, server) {
    ...
    if (config.local) {
      createTestServer(config, function (err, url) {
        if (err) return cb(err)
    ...
    if (config.phantom) {
      ...
      return phantom.start(function (err, stats) {
    ...
    if (config.electron) {
      ...
      return electron.start(function (err, stats) {
        if (err) return exit(err)
        exit(null, stats.failed === 0 && stats.passed > 0)
      })
    }
    ...
    self._browsers.forEach(function (browser) {
      ...
      batch.push(function (done) {
        browser.start(function end (err, stats) {
          if (err) {
            // TODO: a shutdown error should not trigger a retry.
            // Perhaps move retry handling to AbstractBrowser?
            if (--retries >= 0) {
            ...
            // TODO: can we instead call done(err)?
