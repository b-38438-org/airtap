...
function SauceBrowser (opt, conf) {
  if (!(this instanceof SauceBrowser)) {
    return new SauceBrowser(opt, conf)
  }

  AbstractBrowser.call(this, opt, conf)
}
...
SauceBrowser.prototype.start = function () {
  ...
  self.controller = createTestServer(self._opt, function (err, url) {
    ...
    // TODO: is the event argument used anywhere?
    self.emit('init', self._conf)

    var initOpts = Object.assign({
      build: process.env.TRAVIS_BUILD_NUMBER,
      name: self._opt.project,
      tags: self._opt.tags || [], // TODO: not defined in either conf or opts
      browserName: self._conf.browser,
      version: self._conf.version,
      platform: self._conf.platform
    ...
    if (self._conf.firefox_profile) {
      var fp = new FirefoxProfile()
      var extensions = self._conf.firefox_profile.extensions
      for (var preference in self._conf.firefox_profile) {
        if (preference !== 'extensions') {
          fp.setPreference(preference, self._conf.firefox_profile[preference])
        }
      }
